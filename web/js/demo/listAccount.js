function showBan(email){
      document.getElementById("email").value = email;
    const formInclude = document.querySelector('.form__include');
    if(formInclude.classList.contains("active")){
        formInclude.classList.remove('active')
    }else{
        formInclude.classList.add('active')
    }
    
}
    
    function showUnBan(email){
      document.getElementById("emailUnban").value = email;
      console.log(document.getElementById("emailUnban").value);
    const formInclude = document.querySelector('.form__include-unban');
    if(formInclude.classList.contains("active")){
        formInclude.classList.remove('active')
    }else{
        formInclude.classList.add('active')
...
Validator.isRequired = function (selector, message) {
    return {
        selector: selector,
        test: function (value) {
            return value.trim() ? undefined : message || "Vui lòng không nhập khoảng cách!"
        }
    }
}
...
Validator.isPassword = function (selector, message) {
    return {
        ...
        test: function (value) {
            ...
            return value.trim().includes(' ') ? undefined : message 
        }
    }
}
...
function checkPassword() {
    ...
    const password_confirmation = document.querySelectorAll('.repassWord');
    const invalidMessage = document.querySelectorAll('.passWord_message')

    console.log(invalidMessage)
    if(password.includes(' ')){
        invalidMessage.forEach(invalidMessage => invalidMessage.classList.add('invalid'))
    }else{
        invalidMessage.forEach(invalidMessage => invalidMessage.classList.remove('invalid'))

    }
    if (password === "" || password.length < 6  ) {
       
        //  password_confirmation.classList.remove('active');
        password_confirmation.forEach(password_confirmation => password_confirmation.classList.remove('active'))
    } else {
    // password_confirmation.classList.add('active');
    password_confirmation.forEach(password_confirmation => password_confirmation.classList.add('active'))
