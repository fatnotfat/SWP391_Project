 ...
 * @author Admin
 */
@WebServlet(name = "AdminOrdersTotalSearchByMonthYearServlet", urlPatterns = {"/AdminOrdersTotalSearchByMonthYearServlet"})
public class AdminOrdersTotalSearchByMonthYearServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = siteMaps.getProperty(
                MyApplicationConstants.AdminOrdersTotalSearchByMonthYearServlet.ADMIN_PAGE);
        ...
        try {
            if (dateOrder == null || dateOrder.isEmpty()) {
                ...
                LocalDate dateSep = LocalDate.parse(
                        dateOrder, DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                int year = dateSep.getYear();
                int month = dateSep.getMonthValue();
                OrdersDAO dao3 = new OrdersDAO();
                dao3.showTotalOrdersByMonthAndYear(year, month);
                List<OrdersDTO> result3 = dao3.getOrdersList3();
                if (result3 == null) {
                    dao3.showTotalOrdersByMonth();
                    result3 = dao3.getOrdersList3();
                    request.setAttribute("TOTALORDERSINMONTH_RESULT", result3);
